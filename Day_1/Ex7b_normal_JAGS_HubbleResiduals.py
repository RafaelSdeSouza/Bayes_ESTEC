...
parameters{
    real beta0;                      # linear predictor coefficients
    real beta1;
    real beta2;
    real beta3;
...
transformed parameters{
    ...
    for (i in 1:N) {
        if (type[i] == type[1]) mu[i] = beta0 + beta1 * x[i];
        else mu[i] = beta2 + beta3 * x[i];
    }
}
model{
    # shared hyperprior
    ...
    # priors
    beta0 ~ normal(mu0, sig0);
    beta1 ~ normal(mu0, sig0);
    beta2 ~ normal(mu0, sig0);
    beta3 ~ normal(mu0, sig0);
...
fit.plot(['beta0', 'beta1', 'beta2', 'beta3'])
plt.tight_layout()
plt.show()
