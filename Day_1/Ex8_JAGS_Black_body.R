...
#
# Estimation of black-body temperature 
...
pi <- 3.141592654
c <- 2.997e10 * 1e4                 # velocity of light (cm/s)
k <- 8.6173303e-5                   # Boltzman constant (eV/K)
Temp <-  5777                       # black body temperature (K) 

sd <- 0.25                          # lognormal standard deviation

mu <- (8*pi*h*c/lambda^5)*1/(exp(h*c/(k*Temp*lambda)) - 1)      #  Planck law
muL <- log(mu) - 0.5*log(1 + (sd/mu)^2)                         #  lognormal mean

y <- rlnorm(nobs, muL, sdlog = sd)        # create y as  random normal variate, 
                                          # not quite right as I explain later
    
plot(lambda,y)                           # plot data
...
print(normfit, intervals = c(0.025, 0.975), digits = 2)                     # print results

# print temperature posterior intervals
