...
x = uniform.rvs(loc=-2, scale=7, size=nobs)         # random uniform variable
mu = -1 + 6 * x - 3 * pow(x, 2)                     # linear predictor
y = norm.rvs(loc=mu, scale=1.5)                     # create y as adjusted random variate
...
results = smf.glm(formula='y ~ x + 1', data=my_data, family=sm.families.Gaussian()).fit()
